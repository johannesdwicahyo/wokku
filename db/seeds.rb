...
%w[postgres mysql mariadb].each do |db_type|
  ServiceTier.find_or_create_by!(name: "mini", service_type: db_type) { |t| t.price_cents_per_hour = 0; t.spec = { storage_gb: 1, connections: 20, backups: "manual (2 max)" } }
  ServiceTier.find_or_create_by!(name: "basic", service_type: db_type) { |t| t.price_cents_per_hour = 0.137; t.spec = { storage_gb: 4, connections: 50, backups: "daily, 7-day retention" } }
  ServiceTier.find_or_create_by!(name: "standard", service_type: db_type) { |t| t.price_cents_per_hour = 0.5479; t.spec = { storage_gb: 16, connections: 120, backups: "daily, 14-day retention" } }
end

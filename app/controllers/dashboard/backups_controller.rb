module Dashboard
  class BackupsController < BaseController
    def index
      @database = DatabaseService.find(params[:addon_id])
    ...
    def create
      ...
      if @database.backup_limit_reached?
        redirect_to dashboard_addon_backups_path(@database),
      ...
      redirect_to dashboard_addon_backups_path(@database), notice: "Backup started..."
    end
    ...
    def restore
      ...
      begin
        ...
        redirect_to dashboard_addon_backups_path(backup.database_service), notice: "Restore completed"
      rescue => e
        redirect_to dashboard_addon_backups_path(backup.database_service), alert: "Restore failed: #{e.message}"
      end
    end
  end
end

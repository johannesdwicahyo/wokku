...
MAINT_URL="${WOKKU_MAINT_URL:-https://raw.githubusercontent.com/johannesdwicahyo/wokku.dev/main/scripts/dokku-maintenance.html}"
if [ -d "$(dirname "$MAINT_TEMPLATE")" ]; then
  if curl -fsSL "$MAINT_URL" -o "$MAINT_TEMPLATE.new"; then
    mv "$MAINT_TEMPLATE.new" "$MAINT_TEMPLATE"
    ...
    log "Wokku-branded maintenance page installed (from $MAINT_URL)"
  else
    warn "Failed to fetch maintenance template from $MAINT_URL — leaving Dokku default"
  fi
fi

...
# look on re-provisions.
# ══════════════════════════════════════════════════════════════════
MAINT_TEMPLATE=/var/lib/dokku/plugins/enabled/maintenance/templates/maintenance.html
if [ -d "$(dirname "$MAINT_TEMPLATE")" ]; then
  cat > "$MAINT_TEMPLATE" <<'WOKKU_MAINT'
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
WOKKU_MAINT
  chmod 644 "$MAINT_TEMPLATE"
  chown dokku:dokku "$MAINT_TEMPLATE"

  # Refresh per-app copies so existing apps get the new look too.
  for dir in /home/dokku/*/maintenance; do
    [ -d "$dir" ] || continue
    cp "$MAINT_TEMPLATE" "$dir/maintenance.html"
    chown dokku:dokku "$dir/maintenance.html"
  done
  log "Wokku-branded maintenance page installed"
fi
